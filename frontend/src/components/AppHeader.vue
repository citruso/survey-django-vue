...
#header-wrap
  header
    nav
      router-link#logo.btn(to="/")
      #user.text(v-if="$store.getters.isAuth")
        | Account: {{ $store.state.user.username }}
        i.btn-circular.bi-door-open(@click="exit")
      #buttons(v-else)
        router-link#login.btn-rect(to="/login") Войти
        router-link#register.btn-rect(to="/register") Зарегистрироваться
</template>
